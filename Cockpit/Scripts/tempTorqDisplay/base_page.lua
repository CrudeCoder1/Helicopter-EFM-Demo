...
TurbineOutletTemp.element_params  = {"VIDS_TOT"}
...
Torque.formats         = {"%.0f"} 
Torque.element_params  = {"VIDS_TRQ"}
...
function addSegment(side, pos, param, minVal, material)
	...
	element.controllers  	= {{"parameter_in_range",0, minVal,2000}}
...
for i = 18,74,2  do -- Torque 16-74 (2 psi increment)
	...
	addSegment('R',-8*segmentSize+ i/2*segmentSize, {"VIDS_TRQ"}, i, mat)
end
...
for i = 275,975,25  do -- TOT (25 deg increment)
	...
	addSegment('L',-10*segmentSize+ i/25*segmentSize, {"VIDS_TOT"}, i, mat)
end
